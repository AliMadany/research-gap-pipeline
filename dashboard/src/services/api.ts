...
class ApiService {
  ...
  async deleteArticle(id: string): Promise<{ success: boolean; message: string }> {
    return this.request<{ success: boolean; message: string }>(`/articles/${id}`, {
      method: 'DELETE',
    });
  }
  ...
  // WordPress Posts
  async getWordPressPosts(perPage: number = 20, page: number = 1): Promise<{
    posts: WordPressPost[];
    total_posts: number;
    page: number;
    per_page: number;
    ...
    return this.request<{
      ...
      page: number;
      per_page: number;
    }>(`/wordpress/posts?per_page=${perPage}&page=${page}`);
  }
